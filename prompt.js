...
// Context compression helpers (Ide 1: Structured Context Compression)
function compressPortfolio(p) {
  if (!p || typeof p !== "object") return "Unknown";
  const sol = p.sol != null ? `${Number(p.sol).toFixed(2)} SOL` : "?";
  const usd = p.usd != null ? `$${Number(p.usd).toFixed(2)}` : "?";
  const wallet = p.wallet ? `${String(p.wallet).slice(0, 8)}...` : "?";
  return `{sol: ${sol}, usd: ${usd}, wallet: ${wallet}}`;
}

function compressPositions(pos) {
  if (!pos) return "No positions data";
  const arr = Array.isArray(pos) ? pos : (pos.positions || []);
  if (arr.length === 0) return "0 open positions";
  const summary = arr.slice(0, 5).map((p, i) => {
    const name = p.pool_name || p.pair || "?";
    const amt = p.amount_sol != null ? `${Number(p.amount_sol).toFixed(2)} SOL` : "?";
    const pnl = p.pnl_pct != null ? `${p.pnl_pct > 0 ? "+" : ""}${Number(p.pnl_pct).toFixed(2)}%` : "?";
    const range = p.bin_range ? `bin ${p.bin_range}` : (p.active_bin != null ? `bin ${p.active_bin}` : "?");
    const status = p.status || (p.in_range ? "IN_RANGE" : "OOR");
    return `(${i+1}) ${name}: ${amt} | ${pnl} | ${range} | ${status}`;
  });
  const more = arr.length > 5 ? ` +${arr.length - 5} more` : "";
  return `${arr.length} open:\n${summary.join("\n")}${more}`;
}

function compressStateSummary(ss) {
  if (!ss || typeof ss !== "object") return "No memory data";
  const pools = ss.total_pools_seen || ss.pools_seen || 0;
  const blacklist = ss.blacklist_count || ss.blacklisted || 0;
  const recent = ss.recent_pool ? ss.recent_pool : "?";
  return `{pools_seen: ${pools}, blacklist: ${blacklist}, recent: ${recent}}`;
}
...
export function buildSystemPrompt(agentType, portfolio, positions, stateSummary = null, lessons = null, perfSummary = null, weightsSummary = null, decisionSummary = null, patternSummary = null, discordSignalsSummary = null, trendingDiscoverySummary = null) {
  ...
  if (agentType === "MANAGER") {
    const portfolioCompact = compressPortfolio(portfolio);
    const positionsCompact = compressPositions(positions);
...
Open Positions: ${positionsCompact}
...
Portfolio: ${compressPortfolio(portfolio)}
Open Positions: ${compressPositions(positions)}
Memory: ${compressStateSummary(stateSummary)}
Performance: ${perfSummary ? (typeof perfSummary === "object" ? JSON.stringify(perfSummary) : perfSummary) : "No closed positions yet"}

Config (key): ${compressConfig()}
...
All candidates are PRE-SCREENED by a deterministic scorer. Each candidate has a \`prescreen:\` line showing:
- score: final score (higher = better)
- raw: raw metric score (before memory/wallet adjustments)
- mem: pool memory adjustment (+/-, based on past win rate and cooldowns)
- wallet: smart wallet bonus (cap 15)

Your job: use the prescreen score as a STRONG starting signal, then apply narrative judgment and audit checks to pick the highest-conviction candidate. Candidates are already ranked — the top scorer passed memory + wallet enrichment, so you do NOT need to re-fetch those.
active_bin is pre-fetched. Call deploy_position directly.
...
- rugpull flag from OKX → already penalized in prescreen score; default to SKIP unless smart wallets present
- wash trading flag from OKX → treat as disqualifying even if other metrics look attractive
- PVP symbol conflict (same exact symbol across multiple mints) → major negative. Avoid unless the setup is exceptional.
...
${weightsSummary ? `${weightsSummary}\nPrioritize candidates whose strongest attributes align with high-weight signals.\n\n` : ""}${lessons ? `LESSONS LEARNED:\n${lessons}\n` : ""}${patternSummary ? `═══════════════════════════════════════════
 YOUR OWN TRACK RECORD (learn from this!)
═══════════════════════════════════════════
${patternSummary}
