...
// Core autonomous tools — ALWAYS included in GENERAL role regardless of intent
...
const INTENT_PATTERNS = [
  { intent: "decisions",   re: /\b(why did you|why'd you|why was (?:this|that|it)|what made you|what was the reason|why no deploy|why didn't you deploy|why did you close|why did you deploy|why did you skip|kenapa|alasannya|keputusan)\b/i },
  { intent: "deploy",      re: /\b(deploy|open|add liquidity|lp into|invest in|buka posisi|tambah likuiditas|mulai invest|pasang)\b/i },
  { intent: "close",       re: /\b(close|exit|withdraw|remove liquidity|shut down|tutup|keluar|tarik|cabut|hentikan|stop posisi)\b/i },
  { intent: "claim",       re: /\b(claim|harvest|collect|klaim|ambil|panen)\b.*\b(fee|biaya|reward|hasil)\b/i },
  { intent: "swap",        re: /\b(swap|convert|sell|exchange|tukar|jual|konversi)\b/i },
  { intent: "selfupdate",  re: /\b(self.?update|git pull|pull latest|update (the )?bot|update (the )?agent|update yourself|perbarui diri|update agent|update bot)\b/i },
  { intent: "blocklist",   re: /\b(blacklist|block|unblock|blocklist|blocked deployer|rugger|block dev|block deployer|blokir|daftar hitam)\b/i },
  { intent: "config",      re: /\b(config|setting|threshold|update|set |change|ubah|atur|settingan|konfigurasi)\b/i },
  { intent: "balance",     re: /\b(balance|wallet|sol|how much|saldo|isi wallet|berapa sol|cek saldo|cek balance)\b/i },
  { intent: "positions",   re: /\b(position|portfolio|open|pnl|yield|range|posisi|portofolio|terbuka)\b/i },
  { intent: "strategy",    re: /\b(strategy|strategies|strategi|rencana)\b/i },
  { intent: "screen",      re: /\b(screen|candidate|find pool|search|research|token|cari pool|scan|skrining)\b/i },
  { intent: "memory",      re: /\b(memory|pool history|note|remember|ingat|catat|riwayat pool)\b/i },
  { intent: "smartwallet", re: /\b(smart wallet|kol|whale|watch.?list|add wallet|remove wallet|list wallet|tracked wallet|check pool|who.?s in|wallets in|add to (smart|watch|kol)|dompet pintar|pantau wallet)\b/i },
...
function getToolsForRole(agentType, goal = "") {
  ...
  // GENERAL: match intent from goal, combine matched tool sets + always include core tools
  ...
  // Fall back to all tools if no intent matched (core tools always present)
  if (matched.size <= CORE_TOOLS.size) return tools.filter(t => !GENERAL_INTENT_ONLY_TOOLS.has(t.function.name));
...
import { getDecisionSummary, getPatternSummary } from "./decision-log.js";
...
export async function agentLoop(goal, maxSteps = config.llm.maxSteps, sessionHistory = [], agentType = "GENERAL", model = null, maxOutputTokens = null, options = {}) {
  const { interactive = false, onToolStart = null, onToolFinish = null, source = null } = options;
  ...
  for (let step = 0; step < maxSteps; step++) {
    ...
    try {
      ...
      const ACTION_INTENTS = /\b(deploy|open|add liquidity|close|exit|withdraw|claim|swap|block|unblock|buka|tutup|keluar|tarik|klaim|tukar|blokir|pasang|hentikan|cabut)\b/i;
      let toolChoice = (step === 0 && (ACTION_INTENTS.test(goal) || mustUseRealTool)) ? "required" : "auto";

      // CHAT source: inject last-step warning when running out of steps
      if (source === "CHAT" && maxSteps - step <= 3) {
        const remaining = maxSteps - step;
        const warnMsg = { role: "user", content: `[SYSTEM] ⚠️ Kamu tinggal ${remaining} langkah lagi. Jika belum bisa selesai, BERIKAN JAWABAN TERBAIK SEKARANG — jangan panggil tool lagi kecuali benar-benar perlu. Output langsung, jangan menunggu.` };
        messages = [...messages, warnMsg];
      }

      for (let attempt = 0; attempt < 3; attempt++) {
        try {
          // DeepSeek-compatible: sanitize messages, drop tool_choice=required
          const isDeepSeek = (process.env.LLM_BASE_URL || "").includes("deepseek");
          if (isDeepSeek && toolChoice === "required") toolChoice = "auto";
