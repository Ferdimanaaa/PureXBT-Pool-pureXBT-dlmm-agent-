        ...
        try {
          ...
          if (mgm.chaseOorUpEnabled !== false && p.active_bin != null && p.upper_bin != null && p.active_bin > p.upper_bin) {
            ...
            if (aboveMin >= (mgm.chaseOorUpMinutes ?? 5) && solPure && chases < (mgm.maxChasesPerPool ?? 2) && !_chaseDispatched.has(p.position) && Date.now() - _pollTriggeredAt >= chCd) {
              ...
              const chasePrompt = `CHASE-UP CHECK: Position ${p.position} (${p.pair}) in pool ${p.pool} has been OUT OF RANGE ABOVE for ${Math.round(aboveMin)} minutes — price pumped above our range. The position is ~100% SOL (no impermanent loss). Chase attempt ${chases + 1}/${mgm.maxChasesPerPool ?? 2}. First evaluate MARKET momentum via get_pool_detail: volume, swap_count, unique_traders, volatility, price_trend and their window changes. If get_pool_detail fails or returns unusable data, retry it ONCE before deciding (transient API hiccups usually recover in seconds). Judge ONLY by market data — do NOT use smart-wallet presence as a reason to chase or to skip (wallets can be bagholding; only live trading activity counts). IF market momentum is still healthy: (1) close_position with reason \"chase_up\", then (2) deploy_position on the SAME pool ${p.pool} below the new price — follow the ACTIVE STRATEGY (hybrid + RANGE DEPTH RULES; a strong pump usually means bid_ask), single-side SOL (amount_y per config, amount_x=0, bins_above=0). IF momentum is broken or volume dried up: just close_position with a normal reason. Act now, no questions.`;
