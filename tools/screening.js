...
export async function getTopCandidates({ limit = 10 } = {}) {
  ...
  // Start both independent operations in parallel
  const discoveryPromise = discoverPools({ page_size: 50 });
  const positionsPromise = import("./dlmm.js").then(({ getMyPositions }) => getMyPositions());
  const [discovery, posResult] = await Promise.all([discoveryPromise, positionsPromise]);
